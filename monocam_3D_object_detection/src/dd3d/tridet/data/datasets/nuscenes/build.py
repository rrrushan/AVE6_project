...
from dd3d.tridet.data import collect_dataset_dicts
from dd3d.tridet.structures.boxes3d import GenericBoxes3D
from dd3d.tridet.structures.pose import Pose
from dd3d.tridet.utils.geometry import project_points3d
from dd3d.tridet.utils.visualization import float_to_uint8_color
...
class NuscenesDataset(Dataset):
    def __init__(self, name, root_dir, datum_names=CAMERA_NAMES, min_num_lidar_points=3, min_box_visibility=0.2):
...
def build_nuscenes_dataset(name, root_dir, min_num_lidar_points=3, min_box_visibility=0.2):
    dataset = NuscenesDataset(
        name, root_dir, min_num_lidar_points=min_num_lidar_points, min_box_visibility=min_box_visibility
    )
    dataset_dicts = collect_dataset_dicts(dataset)
    return dataset_dicts


def register_nuscenes_metadata(dataset_name):
    ...
    metadata.evaluators = ("nuscenes_evaluator", )
